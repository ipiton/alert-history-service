...
// LabelsNotRegexFilter filters alerts by label regex not match (!~)
type LabelsNotRegexFilter struct {
	labels map[string]*regexp.Regexp
	patterns map[string]string // Store original patterns for SQL
}

// NewLabelsNotRegexFilter creates a new labels not regex filter
func NewLabelsNotRegexFilter(params map[string]interface{}) (Filter, error) {
	labels, ok := params["labels"].(map[string]string)
	if !ok {
		return nil, fmt.Errorf("invalid labels_not_regex filter params: expected map[string]string")
	}

	if len(labels) == 0 {
		return nil, fmt.Errorf("labels_not_regex filter requires at least one label")
	}
	...
	return &LabelsNotRegexFilter{
		labels:   compiled,
		patterns: patterns,
	}, nil
}

func (f *LabelsNotRegexFilter) Type() FilterType {
	return FilterTypeLabelsNotRegex
}

func (f *LabelsNotRegexFilter) Validate() error {
	if len(f.labels) == 0 {
		return fmt.Errorf("labels_not_regex filter requires at least one label")
	}
	if len(f.labels) > 20 {
...
func (f *LabelsNotRegexFilter) ApplyToQuery(qb *query.Builder) error {
	// Mark for GIN index usage (labels JSONB field)
	qb.MarkGINIndexUsage()

	// Use PostgreSQL NOT regex operator (!~) on label values
...
func (f *LabelsNotRegexFilter) CacheKey() string {
	...
	return fmt.Sprintf("labels_not_regex:%s", strings.Join(parts, ","))
}
